# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 17:11:53 2020
...
class PostprocessYOLO(object):
    ...
    def _reshape_output(self, output, i):
        ...
        dim3 = 3
        # There are CATEGORY_NUM=13 object categories:
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        """Take in a list of three reshaped YOLO outputs in (height,width,3,18) shape and return
        ...
        for output, mask in zip(outputs_reshaped, self.masks):
            ...
            box, category, confidence = self._process_feats_and_filter_boxes(output, mask)
            boxes.append(box)
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        Keyword arguments:
        output_reshaped -- reshaped YOLO output as NumPy arrays with shape (height,width,3,18)
        ...
        def sigmoid_v(value):
            """Return the sigmoid of the input."""
            return 1.0 / (1.0 + cv2.exp(-value))

        def exponential_v(value):
            """Return the exponential of the input."""
            return cv2.exp(value)
        # too slow, using cv2.exp instead
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        box_class_probs_max = np.max(box_class_probs, axis=-1)
        box_class_probs_max = sigmoid_v(box_class_probs_max)
        #box_class_probs_max = np.expand_dims(box_class_probs_max, axis=-1)
        box_class_scores = box_confidences * box_class_probs_max
        pos = np.where(box_class_scores >= self.object_threshold)
    ...
    # optimize time spent
    def _process_feats_and_filter_boxes(self, output_reshaped, mask):
        def sigmoid_v(value):
            """Return the sigmoid of the input."""
            return 1.0 / (1.0 + cv2.exp(-value))
        ...
        box_class_scores = box_confidence * box_class_probs_max
        pos = np.where(box_class_scores >= self.object_threshold)
        boxes = boxes[pos]
        if boxes.shape[0] == 0:
